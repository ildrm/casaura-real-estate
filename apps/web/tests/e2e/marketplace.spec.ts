...
test("published inventory flows from search and map to detail, favorite, and account", async ({ page }, testInfo) => {
  ...
  const { agencyId } = await registerVerifiedAgencyOwner(page, {
    agencyName: `Marketplace International Neighborhood Property Advisors ${suffix}`,
  ...
  for (let position = 1; position <= 5; position += 1) await uploadImage(page, created.data.id, agencyId, position);
  await apiJson(page, `/api/v1/listings/${created.data.id}/submit`, "POST", {}, agencyId);
  await apiJson(page, `/api/v1/listings/${created.data.id}/publish`, "POST", {}, agencyId);
