...
import { PropertyInquiryForm } from "@/components/marketplace/property-inquiry-form";
import { PropertyReportForm } from "@/components/marketplace/property-report-form";
import { OrganicRecommendations } from "@/components/marketplace/organic-recommendations";
import { Icon } from "@/components/ui/icon";
import { apiMutation, apiQuery, publicAssetUrl, type ApiError } from "@/lib/api-client";
import { formatArea, formatDate, formatMoney } from "@/lib/localization";
  ...
  return <main className="property-detail shell">
    ...
    <div className="property-detail__layout">
      <div className="property-detail__main">
        ...
        <dl className="property-fact-strip"><div><dt>Beds</dt><dd>{listing.bedrooms ?? "—"}</dd></div><div><dt>Baths</dt><dd>{listing.bathrooms ?? "—"}</dd></div><div><dt>Interior</dt><dd>{listing.interior_area ? formatArea(listing.interior_area) : "—"}</dd></div><div><dt>Reference</dt><dd>{listing.reference}</dd></div><div><dt>Status</dt><dd className="verified"><Icon name="shield" /> Published</dd></div></dl>
        ...
        <PropertyReportForm listingId={listing.id} listingUrl={listing.url} />
      ...
      <aside className="property-detail__aside"><section className="agency-contact-card"><span className="agency-monogram">{listing.agency.name.split(/\s+/).map((part) => part[0]).slice(0, 2).join("")}</span><div><h2><Link href={`/professionals/${listing.agency.slug}`} prefetch={false}>{listing.agency.name}</Link></h2>{listing.agency.verified ? <p className="verified"><Icon name="shield" /> Verified agency</p> : <p>Agency profile</p>}</div><p>{listing.agency.short_description ?? "Listing information is supplied by the publishing agency."}</p><PropertyInquiryForm listingId={listing.id} agencyName={listing.agency.name} /></section></aside>
    </div>
    ...
    <OrganicRecommendations listingId={listing.id} />
  </main>;
}
